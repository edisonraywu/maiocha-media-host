...
1. 新增批次收件層：保存原始自然語言、逐商品身分與指定日期、照片來源與 hash；批次商品仍送入同一 items 管線。
2. 六張照片逐張保留來源、品質與排序原因；重複或不適合的照片在 Preview 明示排除建議。跨商品 hash 衝突阻擋。
3. 正式商品新增使用者 crystal_name 必填閘門。缺少時仍可讀圖，但不能生成正式待審文案；不用照片猜身分。
4. 指定日期優先且不自動交換；時間使用一次確認的品牌預設。日期衝突提示，無日期不自動排入正式日曆。
5. 批次 Preview、指定日期範圍批准、單篇選 A/B/C；沿用批准 hash 與遠端撤銷機制，發布仍只讀已批准檔案。
