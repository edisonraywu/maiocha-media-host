...
def render_preview(content: Path, config: dict) -> Path:
    ...
    for path in item_files(content):
        ...
        for key in selection:
            ...
            photo_html.append(f'<figure><img loading="lazy" src="{rel}" alt="{esc(p["source_name"])}"><figcaption>{esc(p["source_name"])}</figcaption></figure>')
        ...
        rows.append({'content_id': item['content_id'], 'product_name': title, 'publish_at': item.get('publish_at'),
                     ...
                     'user_provided': item['user_provided'], 'issues': problems, 'qa_result': qa.get('result')})
...
<h2>{esc(title)}</h2><p class="date">{esc(item.get('publish_at'))} · {esc(item.get('content_type'))}</p>
...
<details><summary>商品資料與需要補充的事項</summary><dl>{metadata}</dl><pre>{esc(problems or '沒有待補事項')}</pre></details></article>''')
...
<p class="notice">{esc(warning or '目前排程使用已設定的日期與時間。')}<br>核准模式：{'人工核准' if config['approval_mode'] else '自動（仍需通過全部檢查）'}。READY 是內容完成；SCHEDULED 才是已排程。</p>
<button onclick="filter('all')">全部</button><button onclick="filter('READY')">待核准</button><button onclick="filter('NEEDS_INFO')">需要補充</button></header>
<main>{''.join(cards) if cards else '<article><h2>還沒有商品照片</h2><p>請將每條實際商品放入 inbox 的獨立資料夾，再執行 baobao prepare。</p><p>沒有照片不會生成示範商品文案，也不會標記 READY。</p></article>'}
