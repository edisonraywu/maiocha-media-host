param(
    ...
    [string]$CampaignPath='media/maiocha-restart-firstpost-mortgage-ltv-2026-09-19'
)
...
$targets+=[pscustomobject]@{asset=$m.media.reel;key="$CampaignPath/reel/reel-24s-7-scenes.mp4";type='video/mp4';range=$true}
...
if($targets.Count-ne10){throw 'Expected 10 assets'}
...
$m|ConvertTo-Json -Depth 30|Set-Content -LiteralPath (Join-Path $bundle 'Manifest\campaign_manifest.json') -Encoding utf8
$hf=Join-Path $bundle 'Hosting';$rf=Join-Path $bundle 'Reports';New-Item -ItemType Directory -Force -Path $hf,$rf|Out-Null
[pscustomobject]@{provider='github_pages';owner_repo='edisonraywu/maiocha-media-host';pages_base_url=$PagesBaseUrl;campaign_path=$CampaignPath;free_plan=$true;credit_card_required=$false;paid_features_enabled=$false;retention='keep_until_publish_success_plus_buffer'}|ConvertTo-Json|Set-Content -LiteralPath (Join-Path $hf 'hosting_config.json') -Encoding utf8
